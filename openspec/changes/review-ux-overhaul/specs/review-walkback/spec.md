...
The engine session SHALL provide a `clear_at(idx)` method that resets a hunk's decision to undecided (nil). The UI SHALL bind this to the `gu` keymap. Clearing a decision SHALL update the sign on the left buffer and the winbar tally.
...
- **AND** the sign on the left buffer for hunk 2 is removed
...
- **AND** the left buffer sign updates to `✓`
...
### Requirement: Explicit submit via gs keymap

The review SHALL NOT auto-finalize when all hunks are decided. The user MUST press `gs` to submit the review. The submit flow SHALL handle undecided hunks gracefully.

#### Scenario: Submit with all hunks decided

- **WHEN** all hunks have decisions (accepted or rejected)
- **AND** the user presses `gs`
