...
The bus SHALL detect dead channels and clean up stale registrations. The bus SHALL NOT modify `vim.g` state — session.lua owns that responsibility. The bus SHALL NOT modify the channels table while iterating over it.
...
#### Scenario: Health check iteration safety
- **WHEN** the health check timer fires
- **AND** multiple channels exist in the registry
- **AND** one or more channels are dead
- **THEN** the bus SHALL collect dead channel names in a temporary list
- **AND** SHALL unregister dead channels after completing iteration
- **AND** SHALL NOT modify the channels table during iteration over pairs()
