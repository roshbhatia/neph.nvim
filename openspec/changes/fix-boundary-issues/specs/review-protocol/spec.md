...
The review flow SHALL be non-blocking to avoid deadlocks in :terminal buffers. The protocol SHALL support both pre-write review (content proposed, not yet on disk) and post-write review (content already on disk, comparing against buffer). The protocol SHALL handle result_path consistently for all agent types.
...
#### Scenario: Extension agent review request
- **WHEN** an extension agent sends `review.open` request
- **THEN** request SHALL include `request_id` (uuid), `result_path`, and `channel_id`
- **AND** SHALL use identical parameter set as CLI agents
- **AND** result_path SHALL be provided for reliable fallback
