...
The review UI SHALL present hunks in a vimdiff tab with buffer-local keymaps for accepting and rejecting individual hunks. Users MUST be able to navigate freely between hunks using standard Vim diff navigation and make decisions in any order.
...
- **AND** the accept sign is placed on hunk 2's start line
- **AND** the cursor jumps to the next undecided hunk

#### Scenario: Reject a hunk with reason via keymap

- **WHEN** a review is open and the cursor is on hunk 1
- **AND** the user presses `gr`
- **THEN** the user is prompted for a rejection reason via `vim.fn.input`
- **AND** hunk 1 is marked as rejected with the provided reason
- **AND** the reject sign is placed on hunk 1's start line
...
- **AND** the review finalizes with the correct envelope

#### Scenario: Reject all remaining undecided hunks

- **WHEN** a review is open with undecided hunks
- **AND** the user presses `gR`
- **THEN** the user is prompted for a rejection reason
- **AND** all undecided hunks are marked as rejected
...
#### Scenario: Auto-finalize when all hunks decided

- **WHEN** the user accepts or rejects the last undecided hunk
- **THEN** the review automatically finalizes
...
#### Scenario: Winbar shows current hunk status

- **WHEN** the cursor is on hunk 2 of 5
- **AND** hunk 2 has been accepted
- **THEN** the winbar displays `[Hunk 2/5: accepted]  ga=accept  gr=reject  gA=all  gR=reject-all`
