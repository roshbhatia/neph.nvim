...
### Requirement: Review request_id propagation through UI state

The review orchestrator SHALL pass `request_id` through to the UI state so that error recovery can properly dequeue reviews.

#### Scenario: Error during review finalization

- **WHEN** the review UI encounters an error during finalization
- **AND** `ui_state.request_id` is accessed for queue cleanup
- **THEN** `ui_state.request_id` SHALL contain the original request_id from the review params
- **AND** `review_queue.on_complete()` SHALL be called with the correct request_id

#### Scenario: Normal review completion

- **WHEN** a review completes normally
- **THEN** `review_queue.on_complete()` SHALL be called with the same request_id that was passed to `review.open()`

### Requirement: Post-write notification accuracy

The fs_watcher notification SHALL accurately describe what is happening, not reference non-existent commands.

#### Scenario: Post-write review opens immediately

- **WHEN** the fs_watcher detects an agent file change
- **AND** no other review is active
- **THEN** the notification SHALL say "Agent changed: <file> — opening review"

#### Scenario: Post-write review is queued

- **WHEN** the fs_watcher detects an agent file change
- **AND** another review is already active
- **THEN** the notification SHALL say "Agent changed: <file> — review queued"
- **AND** the notification SHALL NOT reference `:NephReviewPost`
