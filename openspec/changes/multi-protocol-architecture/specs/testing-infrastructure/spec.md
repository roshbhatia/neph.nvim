...
The system SHALL validate that Lua dispatch and TS client agree on the RPC method catalog.
...
#### Scenario: TS contract test
- **WHEN** running `tools/neph-cli/tests/contract.test.ts`
- **THEN** test SHALL load `protocol.json`
- **AND** assert every CLI command maps to a known method
