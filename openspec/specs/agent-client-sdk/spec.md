...
`NephClient` SHALL provide a `review(filePath, content)` method that opens an interactive review in Neovim and returns the user's decision.
...
- **AND** the promise SHALL resolve with a `ReviewEnvelope` when the user finishes
