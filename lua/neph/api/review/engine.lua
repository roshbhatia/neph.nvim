...
function M.compute_hunks(old_lines, new_lines)
  ...
  if #old_lines > 0 then old_str = old_str .. "\n" end
  if #new_lines > 0 then new_str = new_str .. "\n" end
...
function M.apply_decisions(old_lines, new_lines, decisions)
  ...
  if not ok or not diff_result then return table.concat(old_lines, "\n") end
...
function M.build_envelope(decisions, content)
  local accepted = vim.tbl_filter(function(h) return h.decision == "accept" end, decisions)
  local rejected = vim.tbl_filter(function(h) return h.decision == "reject" end, decisions)
...
function M.create_session(old_lines, new_lines)
  ...
  function self.accept()
    if current_idx > #hunk_ranges then return false end
