...
function unwrapResult(raw: unknown): { ok: boolean; result?: unknown; error?: { code: string; message: string } } {
  if (typeof raw !== 'object' || raw === null) {
    return { ok: false, error: { code: 'UNEXPECTED', message: String(raw) } };
  }
...
export async function runReviewCtrlCommand(
  ...
  switch (subcommand) {
    ...
    case 'accept': {
      ...
      const remaining = r.next != null ? `(next undecided: hunk ${r.next})` : '(no more undecided)';
      process.stdout.write(`\u2713 Hunk ${r.idx} accepted ${remaining}\n`);
      break;
    }
    ...
    case 'reject': {
      ...
      const remaining = r.next != null ? `(next undecided: hunk ${r.next})` : '(no more undecided)';
      process.stdout.write(`\u2717 Hunk ${r.idx} rejected ${remaining}\n`);
      break;
    }
