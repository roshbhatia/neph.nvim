...
* [2026-04-20 17:07:02]: Updated Overview and Architecture to describe composable pipeline. Updated Mermaid diagrams to include Policy Engine. Updated API Endpoints with new methods from `protocol.json` (added `ui.*`, `tools.*`, `review.*` and removed `bus.register`).
